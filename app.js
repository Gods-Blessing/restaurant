const menu = [
  ...
  // Nav filters
  function addListenerOnNav() {
    navItems.forEach((node)=>{
      node.addEventListener('click',(e)=>{
        let textValue = e.target.innerText;
        let filteredItems;
        let filteringField = "category";
        if(textValue === "All"){
          menuContainer.innerHTML = '';
          insertMenuItems();
          return;
        }else{
          filteredItems = menu.filter((item)=> item[filteringField] === textValue.toLowerCase());
        }
        menuContainer.innerHTML = '';
  ...
  searchInput.addEventListener('input', (e)=>{
    ...
    let filteringField = "title";
    filterItems(filteringField,value);
  })

  function filterItems(filteringField, searchParam){
    if(searchParam){
      let filteredItems = menu.filter((item)=> item[filteringField].toLowerCase().includes(searchParam.toLowerCase()));
      menuContainer.innerHTML = '';
      filteredItems.forEach((item)=>{
        returnItems(item);
      })
  ...
  function returnItems(item){
    ...
    div.classList.add('menu-item', 'col-12', 'col-md-6', 'col-lg-4', 'menu-item-width');
    div.innerHTML = `
      <div class="card card-height">
          <img src="${item.img}" class="card-img-top card-img-height" alt="${item.title}">
          <div class="card-body">
